...
class rbm:
    ...
    def train(self, v0, hiddenunits, visibleunits, alpha = 1.0, epochs = 25, batchsize = 100, plot = True):
        ...
        self.W = tf.Variable(tf.random.truncated_normal([self.visibleunits, self.hiddenunits], stddev=0.1))
        ...
        for i in range(epochs):
            for start, end in zip(range(0, len(v0), batchsize), 
            range(batchsize, len(v0), batchsize)):
                batch = v0[start:end]
